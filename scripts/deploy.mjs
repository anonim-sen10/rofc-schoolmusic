...
function logSection(title) {
    ...
    console.log(
        `${colors.bright}${colors.blue}${title.padEnd(60)}${colors.reset}`,
    );
    console.log(
        `${colors.bright}${colors.blue}${"=".repeat(60)}${colors.reset}\n`,
    );
...
echo "✅ Deployment completed successfully!"
    `.trim();
        ...
        if (config.DEPLOY_SSH_PASSPHRASE) {
            log(
                "warn",
                "Note: SSH passphrase configured, you may be prompted.",
            );
        }
...
async function main() {
    try {
        ...
        console.log(
            `${colors.green}${colors.bright}All done! Your application is live.${colors.reset}\n`,
        );
    } catch (error) {
        logSection("❌ DEPLOYMENT FAILED");
        console.error(
            `${colors.red}${colors.bright}Deployment encountered an error.${colors.reset}\n`,
        );
        process.exit(1);
    }
}
